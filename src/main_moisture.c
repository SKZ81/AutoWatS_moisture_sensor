...
uint8_t i2c_get_version(uint8_t *buffer, uint8_t buffer_len) {
    ...
    buffer[0] = EEPROM_LOCATION_FW_VERSION_MAJOR;
    buffer[1] = EEPROM_LOCATION_FW_VERSION_MINOR;
...
int main (void) {
    ...
    excitation_freq_index = eeprom_read_byte(EEPROM_LOCATION_EXCITATION_FREQ_INDEX);
    ...
    uint8_t fw_version_major = eeprom_read_byte(EEPROM_LOCATION_FW_VERSION_MAJOR);
    uint8_t fw_version_minor = eeprom_read_byte(EEPROM_LOCATION_FW_VERSION_MINOR);
